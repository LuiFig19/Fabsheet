...
async function sendMagicLinkEmail(email: string, url: string): Promise<void> {
  ...
  const safeUrl = humanMagicLinkUrl(url);
  if (!apiKey) {
    console.log(`[auth] RESEND_API_KEY missing. Would send to ${email}: ${safeUrl}`);
    return;
  }
        ...
        <tr><td align="center">
          <table width="460" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:12px;border:1px solid #e5e7eb;overflow:hidden;">
            ...
            <tr><td style="padding:24px;color:#111827;font-size:15px;line-height:1.5;">
              ...
              <p style="margin:0 0 24px;"><a href="${safeUrl}" style="background:#0A1929;color:#fff;padding:12px 20px;border-radius:8px;text-decoration:none;font-weight:500;display:inline-block;">Sign in to ${PRODUCT}</a></p>
              <p style="margin:0 0 8px;color:#6b7280;font-size:13px;">Or paste this link:</p>
              <p style="margin:0;color:#374151;font-size:12px;word-break:break-all;">${safeUrl}</p>
            </td></tr>
          </table>
        </td></tr>
      </table>`,
...
/**
 * BetterAuth magic-link tokens are intentionally single-use. Some mail clients
 * and security scanners GET the target URL before the human clicks it, which
 * consumes the token and leaves the user with INVALID_TOKEN. Send users to a
 * harmless first-party page instead; that page forwards to BetterAuth only when
 * opened in a real browser.
 */
function humanMagicLinkUrl(verifyUrl: string): string {
  const safe = new URL("/magic-login", BASE_URL);
  safe.searchParams.set("verify", verifyUrl);
